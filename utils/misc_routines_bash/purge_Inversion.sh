...
if [[ ${mode} == "deep" ]]; then
    ...
    tempFName=$(bash "${plainParamsFile}" -a get -m "${master}" -F binaryFNames -f projInput -r)
    [[ -e ${tempFName} ]] && rm "${tempFName}"    

    tempFName=$(bash "${plainParamsFile}" -a get -m "${master}" -F binaryFNames -f projOutput -r)
    [[ -e ${tempFName} ]] && rm "${tempFName}"    

    tempFName=$(bash "${plainParamsFile}" -a get -m "${master}" -F binaryFNames -f estimateSino -r)
    [[ -e ${tempFName} ]] && rm "${tempFName}"    

    tempFName=$(bash "${plainParamsFile}" -a get -m "${master}" -F binaryFNames -f consensusRecon -r)
    [[ -e ${tempFName} ]] && rm "${tempFName}"    

    tempFName=$(bash "${plainParamsFile}" -a get -m "${master}" -F binaryFNames -f jigMeasurementsSino -r)
    [[ -e ${tempFName} ]] && rm "${tempFName}"    

    tempFName=$(bash "${plainParamsFile}" -a get -m "${master}" -F binaryFNames -f backprojlikeInput -r)
    [[ -e ${tempFName} ]] && rm "${tempFName}"  

    tempFName=$(bash "${plainParamsFile}" -a get -m "${master}" -F binaryFNames -f backprojlikeOutput -r)
    [[ -e ${tempFName} ]] && rm "${tempFName}"  

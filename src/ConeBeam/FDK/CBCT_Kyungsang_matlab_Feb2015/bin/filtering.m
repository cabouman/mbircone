...
% us = ((-param.nu/2+0.5):1:(param.nu/2-0.5))*param.du + param.off_u;   % Removed by Thilo
% vs = ((-param.nv/2+0.5):1:(param.nv/2-0.5))*param.dv + param.off_v;   % Removed by Thilo
us = param.us;                                                          % Added   by Thilo
vs = param.vs;                                                          % Added   by Thilo
...
for i=1:param.nProj
    ...
    fproj(round(filt_len/2-param.nu/2+1):round(filt_len/2+param.nu/2),:) = proj(:,:,i);
    ...
    if param.parker == 1
        proj(:,:,i) = fproj(round(end/2-param.nu/2+1):round(end/2+param.nu/2),:)/2/param.du*(2*pi/(180/param.dang))/2*(param.DSD/param.DSO);
    else
        proj(:,:,i) = fproj(round(end/2-param.nu/2+1):round(end/2+param.nu/2),:)/2/param.du*(2*pi/param.nProj)/2*(param.DSD/param.DSO);
    end
end

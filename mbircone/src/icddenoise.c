...
void ICDStep3DConeDenoise(struct Image *image, struct Image *err_image, struct ICDInfo3DCone *icdInfo, struct ReconParams *reconParams)
{
    /**
    ...
    err_image->vox[index3D(icdInfo->j_x, icdInfo->j_y, icdInfo->j_z, err_image->params.N_y, err_image->params.N_y)] -= icdInfo->Delta_xj; 
}
...
void computeTheta1Theta2ForwardTermDenoise(struct Image *err_image, struct ICDInfo3DCone *icdInfo, struct ReconParams *reconParams)
{
    ...
    j_y = icdInfo->j_y;
    j_z = icdInfo->j_z;
    icdInfo->theta1_f = -err_image->vox[index_3D(j_x, j_y, j_z, err_image->params.N_y, err_image->params.N_y)]/reconParams->weightScaler_value; 
...
float MAPCost3DDenoise(struct Image *image, struct Image *err_image, struct ReconParams *reconParams)
{
    ...
    cost = MAPCostForward(err_image, reconParams);

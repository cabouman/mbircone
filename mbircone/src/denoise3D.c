    ...
    for (itNumber = 0; (itNumber <= MaxIterations) && (stopFlag==0); ++itNumber) 
    {
        ...
        if (itNumber>0)
        {
            ...
            for (j_xyz = 0; j_xyz < N_x*N_y*N_z; ++j_xyz)
            {
                 ...
                 */
                ICDStep3DDenoise(img, err_img, &icdInfo, reconParams, &reconAux);
        ...
        if (reconParams->verbosity>0)
            disp_iterationInfo_denoise(itNumber, MaxIterations, cost, relUpdate, stopThresholdChange, reconParams->weightScaler_value, float ticToc_icdUpdate)
    }

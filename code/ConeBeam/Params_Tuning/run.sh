...
matlabCommand="tuneParamsGo('${masterFile}', '${plainParamsFile}'); exit(${matlabSuccess});"
matlab -nojvm -r "${matlabCommand}" < /dev/null
if [[  $? != ${matlabSuccess} ]]; then messageError "tuneParamsGo failed. Command = \"${matlabCommand}\"" ; generalError "$0 $@"; exit 1; fi

...
matlabCommand="write_subVolume_reconList_staticPaths('${fNameList_3D_fName}', '${fName_timeVol}', '${coord}'); exit(${matlabSuccess});"
matlab -nojvm -r "${matlabCommand}" < /dev/null
if [[  $? != ${matlabSuccess} ]]; then messageError "write_subVolume_reconList_staticPaths failed. Command = \"${matlabCommand}\"" ; generalError "$0 $@"; exit 1; fi

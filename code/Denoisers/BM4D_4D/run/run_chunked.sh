...
denoiseScript=$(readlink -f "../sourceCode/denoise.m")
...
time bash "${chunkedDenoise_script}" -M "${chunkedDenoiseMasterFile}" -m "${denoisingMasterFile}" -c "${denoiserCloneFile}" -d "${denoiseScript}" -r "${denoisingMultiRun}" -p "${prefix}" -s "${suffix}" -L "MATLAB"

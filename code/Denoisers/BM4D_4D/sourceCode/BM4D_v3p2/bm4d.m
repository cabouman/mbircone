function [y_est, sigma_est] = bm4d(z, distribution, sigma, profile, do_wiener, verbose, searchWindowSize)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%                                   : 'np' --> normal profile 
%                                   : 'mp' --> modified profile
%                                   : 'bm3d' --> for testing bm3d in 2D natural images
%                                     (default is 'mp')
...
% Set default search window size
if ~exist('searchWindowSize','var') || isempty(verbose)
    searchWindowSize = 11;
end
...
if ~strcmpi(profile,'lc') && ~strcmpi(profile,'np') && ~strcmpi(profile,'mp') && ~strcmpi(profile,'bm3d')
...
Ns                   = searchWindowSize;     % search neighborhood size NsxNsxNs

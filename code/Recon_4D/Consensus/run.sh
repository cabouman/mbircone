...
matlabCommand="consensus_iterations('${masterFile}', '${plainParamsFile}'); exit(${matlabSuccess});"
matlab -nojvm -r "${matlabCommand}" < /dev/null
if [[  $? != ${matlabSuccess} ]]; then messageError "consensus_iterations failed. Command = \"${matlabCommand}\"" ; generalError "$0 $@"; exit 1; fi
